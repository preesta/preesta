...
### Phase 13: Plane support ✅
- Fourth tracker alongside Jira, Linear, and GitHub. New `PlaneRest/` project (mirror of `JiraRest/` in spirit — Plane has no public GraphQL surface) with `IPlaneGateway` + `PlaneConnection`. Three gateway methods: `ListWorkItems` (read), `ListWorkspaceMembers` (UUID → email map for routing), `Send` (raw REST for mutations). Authentication via `X-API-Key: plane_api_<token>` — Plane's custom header, **not** `Authorization: Bearer`
- `PlaneRule` carries a required `projectId` (Plane's list endpoint is project-scoped — there is no org-wide search comparable to GitHub) and an optional `filter` YAML mapping of Plane query-param keys to values (e.g. `priority: urgent,high`, `search: "memory leak"`). Keys match Plane's documented list-issues params verbatim — most human-readable form possible without inventing a tracker-specific DSL. Empty filter = whole project, the natural default for small projects
- `mutations:` use the existing REST shape (`verb` / `urlPattern` / `body`) — Plane is REST-only, so the Jira-style `RestMutationSpec` path applies as-is. No `RestMutationSpec` changes; `PlaneMutationExecutor : IHttpHandler` routes the requests through `PlaneConnection` so the `X-API-Key` header is set
- `YamlRuleEntry.Filter` changed from `string?` to `object?` so Linear/GitHub (string) and Plane (mapping) can share the same `filter:` YAML key. Linear/GitHub converters coerce to string via cast; Plane converter walks the map. No user-facing change for existing rule files
- `PlaneIssueSource` maps Plane work-items into the shared `Issue` model. Field decisions: `Key = #<sequence_id>` (Plane's API doesn't return a project code prefix; `ProjectKey` carries the project UUID so multi-project digests stay distinguishable); `PlaneId` (new on `Issue`) carries the work-item UUID; `{{@issueId}}` marker now falls back through `LinearId → GithubNodeId → PlaneId`; `state_detail.name` → `Status` (with `expand=state` query param) or shortened `state-XXXXXXXX` from the bare UUID otherwise; lowercase `urgent`/`high`/`medium`/`low` → title case; `completed_at != null` → `Resolution = "Completed"`
- Assignee / reporter email resolution: Plane returns user UUIDs in work-item payloads, not embedded user objects. A once-cached `UUID → User` map is fetched via `GET /workspaces/{slug}/members/` on first read; lookup failures (HTTP error, no permission) log a warning and leave the map empty — assignees still get a minimal `User` so the digest renders, but `Email=""` so the marker resolver skips routing for them (same degraded path as GitHub's hidden-email case)
- Obezlichennye-rules contract preserved — filter mappings are impersonal (no `assignees: me`-style markers); per-recipient routing lives in `notification.mailTo: assignee` + the workspace-level `slackUsers:` / `telegramUsers:` (email→ID) maps
- DI registers a `"Plane"` keyed pipeline iff **both** `Plane:apiKey` and `Plane:workspaceSlug` are set; `Plane:apiBase` is optional (defaults to `https://api.plane.so/` for Plane Cloud, override for self-hosted). `Application.cs` resolves via `TryResolveNotificationPipe` — no behaviour change when unconfigured. `ResolvePlaneWebRoot` maps `api.plane.so → app.plane.so` for the digest URL root; self-hosted setups serve API + UI off the same host so it passes through unchanged
- Digest header gets a new `Plane filter: priority=urgent, search=leak` line (or `Plane project: <id>` when the filter is empty) via the same `LinearFilterDescriptionOrNull` helper used for Linear/GitHub
- Tests: 150 → 179 (17 `PlaneIssueSourceTests` mapping + 4 `PlaneMutationExecutorTests` + 3 `PlaneIssueSupplierTests` for Enrich + 5 `YamlConfigTests` for plane-rule parsing)
- **Members endpoint caveat** — Plane's public API doc doesn't formally document `/workspaces/{slug}/members/`; some older self-hosted Plane versions expose it only via the internal session-authenticated UI route. If members listing returns 404/403 on self-hosted, assignee email routing degrades gracefully (warning logged, `Email=""`); the digest itself still goes out via any direct addresses or `slackUsers:` map entries the rule set
